...
subsequent adjoint pass which is run backward in simulation time. Figure \ref{fig:dataflow}
...
on. \cite{plessix2006review} The data access pattern from Figure \ref{fig:dataflow} applies here as well, since every step during the
...
Domain decomposition where the domain of the simulation is distributed over
multiple nodes so as to fit within the memory available on each node. This introduces the overhead
of network communication and might not efficiently use the computational resources on each node.\cite{virieux2009seismic}
...
boundaries when required \cite{clapp2009reverse}\cite{yang2014rtm}. Another commonly used strategy that is applied across
...
by rerunning the forward pass. Starting from Revolve \cite{griewank2000algorithm}, which is a provably optimal
scheduling algorithm to implement this strategy, various studies have used and extended this
technique to various scenarios \cite{wang2009minimal} \cite{aupy2016optimal} \cite{schanen2016asynchronous}.
...
data compression. Well-known general purpose data compression algorithms like
Zlib (which is a part of gzip) \cite{deutsch1996zlib}, and compression for video and image
data such as JPEG-2000 \cite{skodras2001jpeg}have been used for scientific data. More
recently, special purpose compression algorithms for floating-point scientific data have been
developed, such as ZFP or SZ \cite{lindstrom2014fixed} \cite{di2018efficient} 
\cite{Kaklamanis:2012aa} \cite{ratanaworabhan2006fast}.
...
GPU and a host computer. There has been some work on streaming compression
of floating-point data for this specific intent. \cite{gpu-compression}
...
This scenario has been discussed in different studies before - they look at Revolve as an alternative
to compression. This approach was seen for a study of adjoint-based methods on the Navier-Stokes
equation \cite{cyr2015towards}. This was also studied in the context of seismic imaging and FWI
to develop tailored compression methods for the wavefield data that is typically seen in this context - 
including a compression method based on DCT \cite{dalmau2014lossy} and one based on adaptive
coarsening \cite{boehm2016wavefield}. This paper focusses on a broader region, of which this scenario
is just one part. In particular, we discuss the scenarios where the forward field can not fit in memory even
after compression, where compression and Revolve would be used in conjunction. 
...
was 25m. This was run for 2500 timesteps, and the final field stored as
...
The uncompressed size of this field was just under 900MB per timestep.
