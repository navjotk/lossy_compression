...
complete implementation first. We have tried to keep the algorithm application
...
in the first few time steps since we consistently saw up to 800x
...
Our current experimental setup has two  that was realised in ZFP was the lack of parallelization
in the decompression functions. The ZFP manual talks about the
difficulty in implementing a parallel mode decompressor.
