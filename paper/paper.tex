...
\cite{cyr2015towards} studied compression for adjoint-based optimisation in the context of the Navier-Stokes
equation, focussing on the scenario where the entire forward field could be stored in memory after
compression. They looked at Revolve as an alternative to compression.

\cite{dalmau2014lossy} also looked at compression as an alternative to Revolve, although in the same context
as this paper - seismic imaging. Their implementation was, however, storing the compressed data to disk so a
lot of their benefits came from more efficient utilization of the low disk bandwidth. Their focus was on developing
and fine-tuning a domain-specific compression algorithm for seismic imaging which was based on discrete
...
TODO Jan check whether SZ supports 4D - it does support up to 4D. 
...
TODO expand to include FWI objective function and explain that the gradient is computed using the adjoint-state method
